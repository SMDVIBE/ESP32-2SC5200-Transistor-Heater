...
> A DIY transistor heater controlled by an **ESP32-C3 Super Mini**, with transistor temperature estimation from a compensated **VBE-related measurement**, closed-loop current control, a **170×320 ST7789 color display**, and adjustable temperature control from **150 °C to 250 °C**.
...
- **VBE** — compensated VBE-related value used by the temperature calculation
...
- Reads base-related and current-sense signals
- Calculates heating current and a compensated VBE-related value
- Estimates transistor temperature during the controlled measurement phase
- Automatically adjusts the operating current target
...
**VBE** shows the compensated VBE-related value used by the temperature calculation.
...
This subtraction compensates the measured base-related signal using the current-related signal.

The resulting compensated value is used by the temperature calculation. The display label **VBE** is kept as a short name for this temperature-sensing value.
...
2. The analog circuit provides **base-related and current-sense signals**.
3. The **ESP32-C3** reads these signals with its ADC.
4. The firmware calculates **current and compensated VBE**.
5. During the measurement phase, the firmware estimates the transistor temperature.
6. The controller adjusts the **operating current target** and regulates the transistor with **PWM**.
...
Under controlled operating conditions, the voltage across this junction changes with temperature.
...
ESP32 measures VBE
        ↓
Firmware estimates temperature
```
...
The main calibration parameters are kept easy to change for the specific transistor used in the project:
...
![ESP32 Transistor Temperature Controller Schematic](Hardware/Schematic_ESP32-Transistor-Temperature-Controller.png)

This schematic shows the **measurement and control electronics** used by the ESP32-C3 controller. The external transistor connections are brought out as **COLLECTOR**, **BASE**, and **EMITTER**. The drawing focuses on the analog measurement, ADC, PWM, display, button, and control connections rather than the complete external power wiring.
...
│
├── Hardware/
│   └── Schematic_ESP32-Transistor-Temperature-Controller.png
│
├── Images/
│   ├── device_overview.jpg
